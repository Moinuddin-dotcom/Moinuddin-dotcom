<div align="center">

  <a href="https://www.linkedin.com/in/md-moinuddin-chowdhury-67098123b/" target="_blank">
    <img src="https://img.shields.io/static/v1?message=LinkedIn&logo=linkedin&label=&color=0077B5&logoColor=white&labelColor=&style=for-the-badge" height="28" alt="LinkedIn" />
  </a>

  <a href="https://wa.me/8801840060700" target="_blank">
    <img src="https://img.shields.io/static/v1?message=WhatsApp&logo=whatsapp&label=&color=25D366&logoColor=white&labelColor=&style=for-the-badge" height="28" alt="WhatsApp" />
  </a>

  <a href="mailto:moinchy7@gmail.com" target="_blank">
    <img src="https://img.shields.io/static/v1?message=Gmail&logo=gmail&label=&color=D14836&logoColor=white&labelColor=&style=for-the-badge" height="28" alt="Gmail" />
  </a>

</div>

<br />

<!-- ==================== BANNER ==================== -->

<div align="center">
  <a href="https://moinuddin-pf.vercel.app/" target="_blank">
    <img 
      src="https://res.cloudinary.com/dvex5rjik/image/upload/v1786748860/WhatsApp_Image_2026-08-15_at_5.01.52_AM_rtc1l1.jpg" 
      width="100%" 
      alt="Md. Moinuddin Chowdhury - Full Stack Developer" 
    />
  </a>
</div>
<br />

<!-- ==================== INTRO ==================== -->

<h1 align="center">Hi, I'm Md. Moinuddin Chowdhury 👋</h1>

<h3 align="center">
  Full-Stack Developer | Next.js • React • Node.js • NestJS
</h3>

<p align="center">
  Building modern, scalable and production-ready web applications.
</p>

<br />

<!-- ==================== ABOUT ME ==================== -->

## 👨‍💻 About Me

I'm a **Full-Stack Developer** from **Chattogram, Bangladesh**, passionate about building modern, responsive and scalable web applications.

I work across both frontend and backend development, with a strong focus on **Next.js, React, TypeScript, Node.js, NestJS, Express.js, PostgreSQL, MongoDB and REST APIs**.

I enjoy turning ideas into real-world products — from designing responsive user interfaces to building APIs, authentication systems, databases and scalable backend architectures.

* 🚀 Full-Stack Developer focused on modern web technologies
* 💻 Building responsive and production-ready web applications
* ⚛️ Strong experience with React.js and Next.js
* 🛠️ Backend development with Node.js, Express.js and NestJS
* 🗄️ Working with PostgreSQL, Prisma, MongoDB and Redis
* 🔐 Experience with authentication, authorization and REST APIs
* 📦 Building scalable and maintainable application architectures
* 🐛 Bug Generator Since 2023 — *"If it works, don't touch it."* 😄
* 📚 Continuously learning new technologies and improving my engineering skills

---

## 🛠️ Tech Stack

### 🎨 Frontend

<div align="left">

<img src="https://skillicons.dev/icons?i=html" height="45" alt="HTML5" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=css" height="45" alt="CSS3" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=javascript" height="45" alt="JavaScript" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=typescript" height="45" alt="TypeScript" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=react" height="45" alt="React" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=nextjs" height="45" alt="Next.js" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=tailwind" height="45" alt="Tailwind CSS" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=redux" height="45" alt="Redux" />

</div>

<br />

### ⚙️ Backend

<div align="left">

<img src="https://skillicons.dev/icons?i=nodejs" height="45" alt="Node.js" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=express" height="45" alt="Express.js" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=nestjs" height="45" alt="NestJS" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=typescript" height="45" alt="TypeScript" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=socketio" height="45" alt="Socket.IO" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=nextjs" height="45" alt="Next.js" />

</div>

<br />

### 🗄️ Database & ORM

<div align="left">

<img src="https://skillicons.dev/icons?i=postgres" height="45" alt="PostgreSQL" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=mongodb" height="45" alt="MongoDB" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=redis" height="45" alt="Redis" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=prisma" height="45" alt="Prisma" />

</div>

<br />

### 🔐 Authentication & APIs

<div align="left">

<img src="https://skillicons.dev/icons?i=firebase" height="45" alt="Firebase" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=postman" height="45" alt="Postman" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=graphql" height="45" alt="GraphQL" />

</div>

<br />

### 🐳 DevOps & Tools

<div align="left">

<img src="https://skillicons.dev/icons?i=git" height="45" alt="Git" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=github" height="45" alt="GitHub" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=docker" height="45" alt="Docker" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=vercel" height="45" alt="Vercel" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=vscode" height="45" alt="VS Code" />
<img width="10" />

<img src="https://skillicons.dev/icons?i=linux" height="45" alt="Linux" />

</div>

---

## 🚀 What I Build

```text
Frontend
├── React.js
├── Next.js
├── TypeScript
├── Tailwind CSS
└── Responsive UI

Backend
├── Node.js
├── Express.js
├── NestJS
├── REST APIs
└── Socket.IO

Database
├── PostgreSQL
├── MongoDB
├── Redis
└── Prisma ORM

Development
├── Git & GitHub
├── Docker
├── Postman
└── Vercel
```

---

## 💼 Featured Project

### 🏪 Guptodhan — Multivendor Marketplace

A full-stack multivendor marketplace platform with multiple business modules and real-time functionality.

**Key Features:**

* 🛒 E-commerce marketplace
* 🏪 Vendor store management
* 🛍️ Buy & Sell marketplace
* 🧑‍💼 Service marketplace
* 💬 Real-time chat using Socket.IO
* 🔐 Authentication & authorization
* 🔎 Search functionality
* 📦 Product & order management
* 💳 Checkout workflow
* 📊 Vendor dashboard
* 🎯 Responsive UI

**Technologies:**

`Next.js` `React` `TypeScript` `Tailwind CSS` `Node.js` `Express.js` `MongoDB` `Socket.IO`

---

## 📊 GitHub Stats

<div align="center">

  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Moinuddin-dotcom&theme=dark" width="100%" alt="GitHub Profile Summary" />

</div>

<br />

<div align="center">

  <img src="https://github-readme-stats.vercel.app/api?username=Moinuddin-dotcom&show_icons=true&theme=dark&hide_border=true" height="180" alt="GitHub Stats" />

  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Moinuddin-dotcom&layout=compact&theme=dark&hide_border=true" height="180" alt="Top Languages" />

</div>

<br />

<div align="center">

  <img src="https://nirzak-streak-stats.vercel.app?user=Moinuddin-dotcom&theme=dark&hide_border=true" alt="GitHub Streak" />

</div>

---

## 📫 Connect With Me

<div align="center">

<a href="https://www.linkedin.com/in/md-moinuddin-chowdhury-67098123b/" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<a href="mailto:moinchy7@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-Contact-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

<a href="https://github.com/Moinuddin-dotcom" target="_blank">
  <img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

</div>

<br />

<div align="center">

### 🚀 "Building ideas into scalable digital products."

</div>
